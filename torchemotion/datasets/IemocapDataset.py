...
def get_transcription_list(path_to_transcriptions):
    f = open(path_to_transcriptions, 'r').read()
    f = np.array(f.split('\n'))
    transcription = []
    for i in range(len(f) - 1):
...
class IemocapDataset(Dataset):
    ...
    def __init__(self,
        ...
        text_paths = [os.path.join(root, 'Session' + file[4], 'dialog', 'transcriptions', file[:-5] + ".txt") for file in self.df['file']]
        self.df['transcription'] = [get_transcriptions(p)[file] for p, file in zip(text_paths, self.df['file'])]
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            # 'path': audio_name,
            'input_values': processor.feature_extractor(waveform, sampling_rate=16000).input_values[0],
            # 'sample_rate': sample_rate,
            'labels': emotion,
            # 'activation': activation,
            # 'valence': valence,
        ...
        # https://fixexception.com/transformers/unable-to-create-tensor-you-should-probably-activate-truncation-and-or-padding-with-padding-true-truncation-true-to-have-batched-tensors-with-the-same-length/
        # sample = BatchEncoding({
        #     # 'path': audio_name,
        ...
        #     # 'dominance': dominance
        # })
        # sample = sample.convert_to_tensors(tensor_type="pt", prepend_batch_axis=True)
